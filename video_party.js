...
// Grab and save the URL parameters passed to this script
var VIDEO_SCRIPT_PARAMETERS = get_script_parameters();
console.log(VIDEO_SCRIPT_PARAMETERS);
...
function get_script_parameters() {
  var jsQueryString = document.currentScript.src;
  var qsReg = new RegExp("[?][^#]*","i");
  var keyValues = { };

  jsQueryString = unescape(jsQueryString);
  ...
  // Do we have multiple parameters or just one?
  var rootArr = jsQueryString.split("&");
  for (i = 0; i < rootArr.length; i++) {
    var tempArr = rootArr[i].split("=");
    if(tempArr.length == 2) { 
      keyValues[tempArr[0]] = tempArr[1];
    } 
  }
...
init_web_video_room = function() {
  ...
  var predefined_room_id = null;

  if (VIDEO_SCRIPT_PARAMETERS.room) {
    predefined_room_id = VIDEO_SCRIPT_PARAMETERS.room;
  } else {
    predefined_room_id = prompt('Enter the room ID to join or create. Copy and paste this room name so you can share it with others.', generateUUID());
  }
